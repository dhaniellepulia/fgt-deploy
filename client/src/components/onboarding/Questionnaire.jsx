...
import { updateCommunitySetting } from "../../api/auth";
...
function Questionnaire() {
  ...
  const [selectedOption, setSelectedOption] = useState(
    user?.communitySettingID === 1 ? "no" : "yes"
  );
  ...
  const handleFinish = async () => {
    try {
      setSaving(true);
      const communitySettingID = selectedOption === "yes" ? 0 : 1;
      await updateCommunitySetting(token, communitySettingID);
      updateUser({ communitySettingID });
    ...
    <div className="min-h-screen text-white flex flex-col items-center justify-center p-6 font-sans">
      <div className="text-center mb-10 max-w-2xl">
        <h2 className="text-yellow-500 font-bold text-2xl mb-4">
          Here to join PNE Community?
        </h2>
        <p className="text-neutral-400 text-sm leading-relaxed">
          By default, you will join PNE Community of players that companies use
          to select their testers. But, you can opt out.
        </p>
      </div>
      ...
      <button
        ...
        disabled={saving}
        className="bg-gradient-to-b from-blue-500 to-blue-700 hover:from-blue-400 hover:to-blue-600 text-white font-bold py-2.5 px-16 rounded-md shadow-lg shadow-blue-900/40 active:scale-95 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
      >
